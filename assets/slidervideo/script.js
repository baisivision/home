(function() {
    function f(b) {
        return "false" === b ? !1 : (b = b.match(/(http:|https:|)\/\/(player.|www.)?(vimeo\.com|youtu(be\.com|\.be|be\.googleapis\.com))\/(video\/|embed\/|watch\?v=|v\/)?([A-Za-z0-9._%-]*)(&\S+)?/)) && /youtu\.?be/g.test(b[3]) ? "youtube" : b && /vimeo/g.test(b[3]) ? "vimeo" : !1
    }
    function g(b) {
    ...
    var k = function() {
        ...
        return function(c) {
            var a = $.Deferred();
            c in b ? b[c] ? a.resolve(b[c]) : a.reject("Preview image not found.") : $("<img>").on("load", function() {
                if (120 === (this.naturalWidth || this.width)) {
                    ...
                    default:
                        b[c] = null,
                        a.reject("Preview image not found.")
                    }
                } else
            ...
            if ($(b.target).hasClass("carousel")) {
                ...
                $(b.target).outerFind("[data-bg-video-slide]").each(function() {
                    ...
                    if (a) {
                        ...
                        c && $.fn.YTPlayer && !$(this).find(".playerBox").length && ($(".container-slide", this).before('<div class="mbr-background-video"></div>').prev().YTPlayer({
                            videoURL: a,
                            containment: "self",
                            showControls: !1,
                            mute: !0
                        }),
                        d.length && $(".YTPOverlay", this).css({
                            opacity: d.css("opacity"),
                            backgroundColor: d.css("background-color")
                        }),
                        $(this).find(".image_wrapper img").css("opacity", "0"),
                        $(this).find(".image_wrapper .mbr-overlay").css("opacity", "0"))) : (h(a, function(a) {
                            b.css("background-image", 'url("' + a + '")').show()
                        }),
                        c && $.fn.vimeo_player && !$(this).find(".playerBox").length && ($(".container-slide", this).before('<div class="mbr-background-video"></div>').prev().vimeo_player({
                            videoURL: a,
                            containment: "self",
                            showControls: !1,
                            mute: !0
                        }),
                        d.length && $(".vimeo_player_overlay", this).css({
                            opacity: d.css("opacity"),
                            backgroundColor: d.css("background-color")
                        })))
                    }
                });
                $(b.target).find(".carousel-item iframe").css({
                    transitionProperty: "opacity",
                    transitionDuration: "1000ms"
                });
                $(this).on("slide.bs.carousel", "section.carousel", function(a) {
                    $(a.target).find(".carousel-item.active .mb_YTPlayer").each(function() {
                        $(this).YTPPause()
                    });
                    $(a.target).find(".carousel-item.active .vimeo_player").each(function() {
                        $(this).v_pause()
                    });
                ...
                $(this).on("slid.bs.carousel", "section.carousel", function(a) {
                    ...
                    $(a.target).find(".carousel-item.active iframe").resize().css("opacity", 1)
                })
            }
        })
}
